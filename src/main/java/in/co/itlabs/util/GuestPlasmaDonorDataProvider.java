...
public class GuestPlasmaDonorDataProvider extends AbstractBackEndDataProvider<PlasmaDonor, Void> {

	private ResourceService resourceService;
	private PlasmaDonorFilterParams filterParams;

	public GuestPlasmaDonorDataProvider(ResourceService resourceService) {
	...
	public int getCount() {
		return resourceService.getPlasmaDonorsCountForGuests(filterParams);
	}

	@Override
	protected Stream<PlasmaDonor> fetchFromBackEnd(Query<PlasmaDonor, Void> query) {
		return resourceService.getPlasmaDonorsForGuests(query.getOffset(), query.getLimit(), filterParams).stream();
	}
